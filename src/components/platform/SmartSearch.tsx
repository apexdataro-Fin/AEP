    ...
    >
      ...
      >
        ...
        <form
          ...
          style={{ padding: "var(--aep-space-lg)", borderBottom: "1px solid var(--aep-border)" }}
        >
        ...
        <div style={{ padding: "var(--aep-space)", borderBottom: "1px solid var(--aep-border)" }}>
          <div className="aep-text-xs aep-text-muted" style={{ marginBottom: "0.5rem" }}>
            Filter by:
          </div>
          <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
            <span className="aep-text-xs" style={{ fontWeight: 600 }}>
              Domain:
            </span>
            {domains.slice(0, 6).map((d) => (
              <button
                key={d}
                onClick={() => setFilters((f) => ({ ...f, domains: toggleFilter(f.domains, d) }))}
                className={`aep-tag`}
                style={{
                  cursor: "pointer",
                  background: filters.domains.has(d) ? "var(--aep-primary-bg)" : undefined,
                  borderColor: filters.domains.has(d) ? "var(--aep-primary)" : undefined,
                }}
              >
                {d.replace(/-/g, " ")}
              </button>
        ...
        {query === "" && searchHistory.length > 0 && (
          <div style={{ padding: "var(--aep-space)" }}>
            <div className="aep-text-xs aep-text-muted" style={{ marginBottom: "0.25rem" }}>
              Recent searches
            </div>
