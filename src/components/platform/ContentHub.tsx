...
export function ContentHub({ title, description, categories, featured, recent }: ContentHubProps) {
  return (
    ...
    >
      <h1 style={{ marginBottom: "0.5rem", fontSize: "var(--aep-font-size-3xl)", fontWeight: 800 }}>
        {title}
      </h1>
      {description && (
        <p className="aep-text-muted" style={{ marginBottom: "var(--aep-space-xl)" }}>
          {description}
        </p>
      )}
      ...
      {featured && featured.length > 0 && (
        <>
          ...
          <div className="aep-grid aep-grid--auto" style={{ marginBottom: "var(--aep-space-xl)" }}>
            {featured.map((item) => (
              <HubCard key={item.id} item={item} featured />
            ))}
          </div>
        </>
      )}
  ...
  const catVariant =
    item.category === "book"
      ...
      : item.category === "course"
        ? "course"
        : item.category === "project" || item.category === "interactive-lab"
          ? "project"
          : item.category === "certification"
            ? "certification"
            : item.category === "career-track"
              ? "career"
              : "default";
      ...
      footer={
        ...
        >
          ...
          {item.progress !== undefined && (
            <ProgressBar value={item.progress} size="sm" showLabel={false} />
          )}
        </div>
      }
    />
...
export function BooksLibrary({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      title="Books Library"
      description="Explore books across all domains"
      categories={[
        { id: "all", label: "All Books", icon: "📚", href: "/books", count: items.length },
      ]}
      featured={items.filter((_, i) => i < 3)}
      recent={items}
    />
  );
}
...
export function CoursesLibrary({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      ...
      description="Structured courses and learning paths"
      categories={[
        { id: "all", label: "All Courses", icon: "🎓", href: "/courses", count: items.length },
      ]}
      featured={items.filter((_, i) => i < 3)}
      recent={items}
    />
...
export function ProjectsLibrary({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      ...
      categories={[
        { id: "all", label: "All Projects", icon: "🛠️", href: "/projects", count: items.length },
      ]}
      featured={items.filter((_, i) => i < 3)}
      recent={items}
    />
...
export function LabsLibrary({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      title="Labs Library"
      description="Interactive labs for hands-on practice"
      categories={[
        { id: "all", label: "All Labs", icon: "🧪", href: "/labs", count: items.length },
      ]}
      featured={items.filter((_, i) => i < 3)}
      recent={items}
    />
...
export function KnowledgeBase({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      ...
      description="Reference materials, glossaries, and technology overviews"
      categories={[
...
export function CareerCenter({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      title="Career Center"
      description="Career tracks, interview prep, and certification guides"
      categories={[
...
export function CertificationCenter({ items }: { items: HubItem[] }) {
  return (
    <ContentHub
      ...
      description="Prepare for cloud, security, and AI certifications"
      categories={[
        {
          id: "microsoft",
          label: "Microsoft",
