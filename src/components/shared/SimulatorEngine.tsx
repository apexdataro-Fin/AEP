  ...
  return {
    "/": {
      ...
      children: {
          ...
          children: {
            engineer: {
              ...
              children: {
                "welcome.txt": {
                  type: "file",
                  content: "مرحباً بك في CloudNova!\nأنت الآن مهندس السحابة الجديد في الفريق.",
                  perms: "-rw-r--r--",
                  owner: "engineer",
                  size: 72,
                },
            ...
            log: {
              ...
              children: {
                nginx: {
                  ...
                  children: {
                    "access.log": {
                      type: "file",
                      content:
                        "192.168.1.10 - [15/Jul/2026:10:23:45] GET / HTTP/1.1 200\n192.168.1.11 - [15/Jul/2026:10:23:46] GET /api HTTP/1.1 500\n192.168.1.10 - [15/Jul/2026:10:23:47] GET / HTTP/1.1 200",
                      perms: "-rw-r--r--",
                      owner: "www-data",
                      size: 160,
                    },
                    "error.log": {
                      type: "file",
                      content:
                        "2026/07/15 10:23:46 [error] 1234#0: *2 connect() failed (111: Connection refused) while connecting to upstream",
                      perms: "-rw-r--r--",
                      owner: "www-data",
                      size: 130,
                    },
                ...
                syslog: {
                  type: "file",
                  content:
                    "Jul 15 10:23:45 cloudnova-prod-01 systemd[1]: Started Nginx.\nJul 15 10:23:46 cloudnova-prod-01 kernel: Out of memory: Kill process 5678",
                  perms: "-rw-r-----",
                  owner: "syslog",
                  size: 140,
                },
              ...
              children: {
                html: {
                  ...
                  children: {
                    "index.html": {
                      type: "file",
                      content: "<h1>CloudNova Platform</h1>",
                      perms: "-rw-r--r--",
                      owner: "www-data",
                      size: 30,
                    },
        ...
        usr: {
          type: "dir",
          children: { local: { type: "dir", children: { bin: { type: "dir", children: {} } } } },
        },
...
export function LinuxTerminalSimulator() {
  ...
  const executeCommand = useCallback(
    (cmd: string) => {
      ...
      appendOutput([
        `engineer@cloudnova:~${state.cwd === "/" ? "" : state.cwd.replace("/home/engineer", "~")}$ ${trimmed}`,
      ]);
      ...
      switch (command) {
        ...
        case "uname": {
          if (args.includes("-a"))
            appendOutput([
              "Linux cloudnova-prod-01 5.15.0-1023-azure #29~20.04.1-Ubuntu SMP x86_64 GNU/Linux",
            ]);
        ...
        case "ls": {
          ...
          if (!node || node.type !== "dir") {
            appendOutput([
              `ls: cannot access '${args[0] || targetPath}': No such file or directory`,
            ]);
            break;
          }
          ...
          if (showAll) {
            ...
            names.sort().forEach((name) => {
              ...
              appendOutput([
                `${perms} 1 ${owner} ${owner} ${String(size).padStart(5)} Jul 15 10:23 ${color}${name}\x1b[0m`,
              ]);
            });
          } else {
            const colored = names
              .sort()
              .map((name) => (children[name].type === "dir" ? `\x1b[34m${name}/\x1b[0m` : name));
        ...
        case "touch": {
          ...
          const parent = getNodeAt(state.fs, parentPath);
          if (!parent || parent.type !== "dir") {
            appendOutput([`touch: cannot touch '${args[0]}': No such file or directory`]);
            break;
          }
          setState((s) => {
            const newFS = JSON.parse(JSON.stringify(s.fs));
        ...
        case "rm": {
          ...
          const node = getNodeAt(state.fs, target);
          if (!node) {
            appendOutput([`rm: cannot remove '${fileArg}': No such file or directory`]);
            break;
          }
          if (node.type === "dir" && !recursive) {
            appendOutput([`rm: cannot remove '${fileArg}': Is a directory`]);
            break;
          }
          setState((s) => {
            const newFS = JSON.parse(JSON.stringify(s.fs));
        ...
        case "find": {
          const searchPath =
            args[0] && !args[0].startsWith("-") ? resolvePath(state.cwd, args[0]) : state.cwd;
        ...
        case "grep": {
          ...
          if (!node || node.type !== "file") {
            appendOutput([`grep: ${args[1]}: No such file or directory`]);
            break;
          }
        ...
        case "df": {
          if (args.includes("-h")) {
            ...
            appendOutput([
              ...
              "/dev/sda1       52428800 33554432 18874368  67% /",
            ]);
          }
          break;
        }
        case "free": {
          if (args.includes("-h")) {
            ...
            appendOutput([
              "              total        used        free      shared  buff/cache   available",
              "Mem:       16384000    4456448    8519680      262144    3407872    10678272",
            ]);
          }
          break;
        }
          ...
          if (args[0] === "status" && args[1]) {
            if (args[1] === "nginx") {
              ...
              appendOutput([
                ...
                "   Active: active (running) since Mon 2026-07-14 08:00:00 UTC",
              ]);
            } else {
        ...
        case "curl": {
          if (args.length === 0) {
            appendOutput(["curl: try 'curl --help' for more information"]);
            break;
          }
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#b06b6b" }}
        />
        <span
          style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#c69c3e" }}
        />
        <span
          style={{ width: "10px", height: "10px", borderRadius: "50%", background: "#5a8f6e" }}
        />
        <span style={{ fontSize: "0.75rem", color: "#9e8e7f", marginLeft: "auto" }}>
          CloudNova Terminal
        </span>
      </div>
      <div
  ...
  if (currentChallenge >= dockerChallenges.length) {
    return (
      ...
      >
        ...
        <h3 style={{ color: "var(--aep-text-primary)", marginBottom: "0.5rem" }}>
          أكملت جميع تحديات Docker!
        </h3>
        <p style={{ color: "var(--aep-text-secondary)", marginBottom: "1.5rem" }}>
          أنت الآن تتقن أساسيات Docker. جرب محاكي Linux Terminal للمزيد.
        </p>
        <button onClick={resetAll} className="aep-btn aep-btn--primary">
          إعادة المحاكي
        </button>
      </div>
    );
  ...
  return (
    ...
    >
      <div style={{ padding: "1.25rem", borderBottom: "1px solid var(--aep-border-light)" }}>
        ...
        <p style={{ margin: 0, color: "var(--aep-text-secondary)", fontSize: "0.9rem" }}>
      ...
      <div style={{ padding: "1.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "0.75rem" }}>
          <input
            ...
            spellCheck={false}
          />
          <button onClick={handleSubmit} className="aep-btn aep-btn--primary">
            تشغيل
          </button>
        </div>
        {feedback && (
...
const k8sScenarios: K8sScenario[] = [
  {
    ...
    symptoms: [
      "kubectl get pods يظهر: api-deployment-v3-xxx 0/1 CrashLoopBackOff",
      "الحاوية تبدأ ثم تموت فوراً",
      "Exit Code: 1",
    ],
    question: "ما هو أول أمر يجب أن تنفذه للتشخيص؟",
    options: [
      {
        text: "kubectl delete pod api-deployment-v3-xxx",
        correct: false,
        feedback: "❌ حذف الـ Pod لن يحل المشكلة — الـ Deployment سيعيد إنشائه.",
      },
      {
        text: "kubectl logs api-deployment-v3-xxx --previous",
        correct: true,
        feedback: "✅ صحيح! سجلات الحاوية السابقة ستظهر سبب الانهيار.",
      },
      {
        text: "kubectl scale deployment api-deployment --replicas=0",
        correct: false,
        feedback: "❌ هذا سيوقف الخدمة بالكامل. ليس خطوة أولى جيدة.",
      },
      {
        text: "kubectl rollout undo deployment/api-deployment",
        correct: false,
        feedback: "⚠️ هذا حل آمن لكن الأفضل أن تفهم السبب أولاً.",
      },
    ],
    commands:
      "kubectl logs api-deployment-v3-xxx --previous\n# Error: Cannot find module '@/config/database'\n# at Object.<anonymous> (/app/server.js:5:22)",
  },
  {
    id: 2,
    ...
    symptoms: [
      "kubectl get pods يظهر: new-app-xxx 0/1 ImagePullBackOff",
      "Status: Waiting",
      "Reason: ErrImagePull",
    ],
    ...
    options: [
      {
        text: "الـ Node ليس لديها موارد كافية",
        correct: false,
        feedback: "❌ Resource shortage يسبب Pending وليس ImagePullBackOff.",
      },
      {
        text: "اسم الصورة أو الـ tag غير صحيح",
        correct: true,
        feedback: "✅ صحيح! ImagePullBackOff يعني أن Kubernetes لا يستطيع سحب الصورة.",
      },
      { text: "الـ Service غير معرّفة", correct: false, feedback: "❌ Service لا تؤثر على Pull." },
      {
        text: "الـ ConfigMap غير موجود",
        correct: false,
        feedback: "❌ ConfigMap تسبب CreateContainerConfigError.",
      },
    ],
    commands:
  ...
  },
  {
    ...
    symptoms: [
      "3 Pods في حالة Pending",
      "kubectl describe يظهر: 0/5 nodes are available: 5 Insufficient memory",
      "الـ Nodes ممتلئة",
    ],
    question: "ما الحل الأفضل؟",
    options: [
      {
        text: "حذف كل الـ Pods وإعادة إنشائها",
        correct: false,
        feedback: "❌ لا يحل المشكلة — ستعود نفس الحالة.",
      },
      {
        text: "تقليل requests.memory في الـ Deployment",
        correct: true,
        feedback: "✅ تقليل الذاكرة المطلوبة أو إضافة Nodes جديدة.",
      },
  ...
  },
  {
    ...
    symptoms: [
      "curl http://EXTERNAL-IP يعيد Connection Refused",
      "kubectl get svc يظهر EXTERNAL-IP لكن لا استجابة",
      "الـ Pods تعمل بشكل طبيعي",
    ],
    question: "أين المشكلة غالباً؟",
    options: [
      { text: "الـ Pods معطلة", correct: false, feedback: "❌ الـ Pods تعمل (Running)." },
      {
        text: "الـ selector في Service لا يتطابق مع labels الـ Pods",
        correct: true,
        feedback: "✅ أشهر خطأ! تأكد أن selector يطابق labels الـ Pods.",
      },
      { text: "الـ Cluster DNS معطل", correct: false, feedback: "❌ لا يؤثر على External IP." },
      {
        text: "الـ kubelet معطل",
        correct: false,
        feedback: "❌ لو كان kubelet معطلاً لن تعمل الـ Pods.",
      },
    ],
    commands:
  ...
  },
  {
    ...
    title: "تسرب موارد",
    story:
      "بعد 3 أيام من النشر، لاحظت أن التطبيق أصبح بطيئاً جداً. الـ CPU usage ارتفع من 15% إلى 95%.",
    ...
    options: [
      ...
      {
        text: "Memory leak في الكود",
        correct: true,
        feedback: "✅ الزيادة التدريجية + OOMKilled = memory leak. افحص الـ heap dump.",
      },
      {
        text: "الـ Node بحاجة لإعادة تشغيل",
        correct: false,
        feedback: "❌ إعادة التشغيل مؤقت. المشكلة في الكود.",
      },
  ...
  if (current >= k8sScenarios.length) {
    return (
      ...
      >
        ...
        <h3 style={{ color: "var(--aep-text-primary)", marginBottom: "0.5rem" }}>
          انتهت جميع السيناريوهات!
        </h3>
        <p style={{ color: "var(--aep-text-secondary)", fontSize: "1.1rem" }}>
          النتيجة: {score}/{k8sScenarios.length}
        </p>
        <p style={{ color: "var(--aep-text-muted)", fontSize: "0.85rem", marginBottom: "1.5rem" }}>
          {score === k8sScenarios.length
            ? "🎯 محترف! أنت جاهز لأي حادثة K8s."
            : score >= 3
              ? "👍 جيد! تدرب أكثر على السيناريوهات."
              : "📚 تحتاج مراجعة. عد للدروس!"}
        </p>
        <button onClick={resetAll} className="aep-btn aep-btn--primary">
          إعادة المحاكي
        </button>
    ...
    >
      <div
        style={{
          ...
          borderBottom: "1px solid var(--aep-border-light)",
          display: "flex",
          alignItems: "center",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{ fontWeight: 700, color: "var(--aep-text-primary)", fontSize: "0.9rem" }}>
            Kubernetes Incident Simulator
          </span>
        </div>
        <span style={{ fontSize: "0.8rem", color: "var(--aep-text-muted)" }}>
          Score: {score}/{k8sScenarios.length}
        </span>
      </div>
      <div style={{ padding: "1.25rem" }}>
          ...
          {k8sScenarios.map((_, i) => (
            <div
              ...
              style={{
                ...
                flex: 1,
                borderRadius: "3px",
                background: completedScenarios.has(i)
                  ? "var(--aep-accent)"
                  : current === i
                    ? "var(--aep-primary)"
                    : "var(--aep-border)",
        ...
        <div
          style={{
            padding: "0.5rem 0.75rem",
            background: "rgba(176, 107, 107, 0.08)",
            borderRadius: "0.5rem",
            marginBottom: "1rem",
            borderLeft: "3px solid var(--aep-danger)",
          }}
        >
          <span style={{ fontWeight: 700, color: "var(--aep-danger)", fontSize: "0.8rem" }}>
            🚨 INCIDENT #{scenario.id}
          </span>
          <h4 style={{ margin: "0.25rem 0", color: "var(--aep-text-primary)", fontSize: "1rem" }}>
            {scenario.title}
          </h4>
          <p style={{ margin: 0, color: "var(--aep-text-secondary)", fontSize: "0.85rem" }}>
            {scenario.story}
          </p>
        </div>
        <div style={{ marginBottom: "1rem" }}>
          <strong style={{ fontSize: "0.85rem", color: "var(--aep-text-primary)" }}>
            الأعراض:
          </strong>
          <ul
            style={{
        ...
        {answered && (
          ...
          >
            <div style={{ fontSize: "0.75rem", color: "#9e8e7f", marginBottom: "0.25rem" }}>
              💻 Terminal Output:
            </div>
            <pre
              style={{
