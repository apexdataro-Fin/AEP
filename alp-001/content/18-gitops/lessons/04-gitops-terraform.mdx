...
spec:
  ...
  # Drift detection: plan every interval, detect changes
  # If drift detected and approvePlan=auto → auto-apply
  
  approvePlan: auto
  
  # Store plan output in ConfigMap for audit
  ...
  # Don't destroy resources when Terraform object is deleted
...
<Quiz>
  <Question
    ...
    options={["Every push", "On a configurable interval (e.g., every 10 minutes)", "Only on PR merge", "Never — drift detection is manual"]}
    correct={1}
