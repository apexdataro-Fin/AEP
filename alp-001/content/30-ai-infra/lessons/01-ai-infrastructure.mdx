...
import {
  ...
  ArchitectureNote, CostNote, ProductionNote, InterviewQuestion
...
apiVersion: containerservice/v1
nodePools:
  - name: gpunp
...
<Quiz>
  <Question
    ...
    options={["Regular node pool", "GPU-enabled node pool with NVIDIA device plugin", "Virtual machines", "App Service"]}
    correct={1}
  />
  
  <Question
    question="What is the most cost-effective inference optimization with least effort?"
    options={["Model distillation", "Quantization (INT8/INT4)", "Training a new model", "Buying more GPUs"]}
    correct={1}
