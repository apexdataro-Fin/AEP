...
<BestPractice>
**Always use virtual environments.** Never install Python packages globally. Each project gets its own isolated environment.
</BestPractice>
...
<ProductionNote>
**Real CloudNova example:** When CloudNova needed to tag 2,000 resources with cost center IDs, a 30-line Python script saved 8 hours of manual clicking. The script took 15 minutes to write and 30 seconds to run. That's the cloud engineering mindset.
</ProductionNote>
...
The security team needs a weekly report of all NSG rules that allow inbound traffic from the internet (0.0.0.0/0).
</ArchitectureNote>
...
filename = f"data/open_nsg_rules_{timestamp}.csv"
...
<Flashcard front="Why use virtual environments (`venv`)?" back="Isolate project dependencies. Avoid conflicts between different projects needing different versions of the same package." />

<Flashcard front="What does `DefaultAzureCredential()` do?" back="Tries multiple authentication methods in order: Managed Identity → Environment variables → Azure CLI login → VS Code login. Write once, works everywhere." />

<Flashcard front="When should a cloud engineer use Python vs Bash?" back="Python: complex logic, API calls, JSON/CSV handling, error handling. Bash: simple CLI commands, file operations, pipeline chains." />
