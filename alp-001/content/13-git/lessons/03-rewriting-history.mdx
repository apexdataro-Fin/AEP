...
**A:** 1. **Immediately rotate the credentials** — the secret is compromised regardless of Git cleanup.
2. Use `git filter-branch` or `git filter-repo` (preferred) to remove the file from all history.
3. Force push main after coordinating with the team.
4. All team members must rebase their branches onto the cleaned main.
