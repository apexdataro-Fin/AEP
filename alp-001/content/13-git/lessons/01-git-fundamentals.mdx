...
Git remembers every version of every file you've ever told it to care about. Instead of saving _what changed_ between versions (like a list of edits), it saves a complete snapshot of all files each time you commit. This makes operations like branching, merging, and reverting incredibly fast.
...
<CodeBlock language="bash" title="First-Time Git Setup">
  git config --global user.name "Alex Chen" git config --global user.email "alex.chen@cloudnova.io"
  git config --global core.editor "code --wait" git config --global init.defaultBranch main git
  config --global pull.rebase true git config --global fetch.prune true
</CodeBlock>
